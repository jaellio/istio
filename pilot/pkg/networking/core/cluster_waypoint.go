...
func (cb *ClusterBuilder) buildWaypointInboundVIPCluster(
	...
	if svc.Hostname.IsWildCarded() {
		localCluster = cb.buildCluster(clusterName, nil, nil,
	...
	applyOutlierDetection(nil, localCluster.cluster, outlierDetection)
	// TODO(grnmeira): check the cluster type instead of hostname
	if !svc.Hostname.IsWildCarded() {
		applyLoadBalancer(svc, localCluster.cluster, loadBalancer, &port, cb.locality, cb.proxyLabels, mesh)
	}
	...
	// DYNAMIC_DNS hosts will always be resolved by the DFP cluster, so it doesn't
	// make sense to use an internal listener upstream socket.
	if svc.Hostname.IsWildCarded() {
...
func (cb *ClusterBuilder) buildWaypointInboundVIP(proxy *model.Proxy, svcs map[host.Name]*model.Service, mesh *meshconfig.MeshConfig) []*cluster.Cluster {
	...
	for _, svc := range svcs {
		for _, port := range svc.Ports {
			// Here we reject services that use UDP. Wildcarded services different than HTTP are
			// also rejected as we need L7 information to handle them with dynamic forward proxying.
			if port.Protocol == protocol.UDP || (svc.Hostname.IsWildCarded() && port.Protocol != protocol.HTTP) {
				continue
			}
